function [W,cost,isi] = iva_l_sos_constrained(X,guess_mat,constraint_type,varargin)
%
...
%
% Example call:
% W=iva_l_sos_constrained(X,guess_mat,0,'rho',0.7)
%
...
if nargin==0
   help iva_l_sos_constrained
   test_iva_l_sos_constrained
   return
end
...
num_W=size(W,1);
mu = (1-Params.rho)*Params.gam.* ones(num_guess,K);
   ...
   for n=1:N      
      ...
      for k=1:K
         ...
         if n <= num_guess
             ...
             mu_old(n,k) = mu(n,k);
             mu_new(n,k)=max(0,mu(n,k) + Params.gam*(Params.rho-dis_wr));
...
function test_iva_l_sos_constrained
...
%% Example call to iva_l_sos_constrained to constrain column of mixing matrix
% with optional input, A

W=iva_l_sos_constrained(X,step',0,'A',A,'verbose',true); % Constrain columns of the mixing matrix
% Visualize results
T1 = zeros(N,N);
for k = 1 : K
   Tk = W(:,:,k)*A(:,:,k);
...
W=iva_l_sos_constrained(X,mean(S(1:3,:,:),3)',1,'A',A,'verbose',true); % Constrain columns of the mixing matrix
% Visualize results
T1 = zeros(N,N);
for k = 1 : K
   Tk = W(:,:,k)*A(:,:,k);
